...
Ubuntu 在 **userdata**（不是 A/B）。桌面 `rootfs.ext4` 约 4 GiB，**不进 Release**（GitHub 太大，而且会清空安卓 `/data`）。用 `linux-mainline/scripts/build-rootfs.sh` 在本机做。
...
也可以先下 Release 启动链，userdata 仍在本机编：
...
仓库里没有 root 密码。编镜像时设 `ROOT_PASSWORD`，或写已忽略的 `linux-mainline/out/root-password`。

桌面：GNOME Speakers + **Built-in Microphone**（WCD9385 AMIC5，对齐安卓 speaker-mic）。
