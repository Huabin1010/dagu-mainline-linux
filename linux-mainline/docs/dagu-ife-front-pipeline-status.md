...
- `#504`（**CSID（CSI Decoder，CSI 解码器）** IPP SOF（Start of Frame，帧起始）重装 HCROP/VCROP（Horizontal/Vertical Crop，水平/垂直裁切），核 `#520`）：`sof_recrop` **n=30** `h=0xa1f0000` `v=0x5bf0000`，`meas` 一直 **`0x0/0xfff`**。仍 **1×8110080**。`camif irq1` `0x1 0xc 0x2 0x1`。一次 **COMP_DONE（composite done，合成完成）** `bus=0x80` `as0=as3=0xff000000/0xff528000`；停时 **as0 仍那组**，**as3=`0xfe800000/0xfed28000` 未消费**。`pix_store=True` `errrec=0` `viol=0` `r0114=0x300`。`g_serial` Device 016 `0525:a4a7` 保住。**证伪**：重写裁切窗解不了第二帧。下一刀是 **WM（Write Master，AXI 写通道）** 把 pending **IMAGE_ADDR** 从 as3 推成 as0，不是再脉冲 RESUME / `CAMIF_EN`。禁止 `ALIGN_UP FRAME_INCR`。禁止刷 `53dcc70`。
- `#505`（Code Aurora Forum 相机栈 `update_wm`：已 EN 时重写 `IMAGE_CFG_0` + `FRAME_INCR` + `CFG` EN|PLAIN，刷 B `#505`）：`en_reload` **n=4** `wm=5` `addr=0xfe528000` `cfg0=0x2940910` `incr=0x294000`。仍 **1×8110080**。`sof_recrop` **n=30** `meas` 一直 **`0x0/0xfff`**。`camif irq1` 仍 `0x1 0xc 0x2 0x1`。停时 **as0=`0xff000000/0xff528000`**，**as3=`0xfe800000/0xfed28000` 未消费**。`pix_store=True` `errrec=0` `viol=0` `wm4=wm5=0x1`。`g_serial` Device 018 `0525:a4a7` 保住。`streamon_rc=0`。**证伪**：重写 `cfg_0`/`incr`/EN 推不动 as3。这 4 次 `en-reload` 是 STREAMON 入队，不是每帧 Code Aurora Forum `update_wm`。后置同一套 ADDR-only 已在 `#501` 交出 **3×3110400**。第二帧卡在 **CAMIF（Camera Interface，相机接口）** 第二次 SOF（Start of Frame，帧起始）之后没有 epoch，不是再改 `wm_update`。禁止再把 Code Aurora Forum `update_wm` 顺序当第二帧刀。禁止 `ALIGN_UP FRAME_INCR`。禁止刷 `53dcc70`。
- `#506`（**MNDS（MN Down Scaler，M/N 下采样器）** Y V 源改 **CSID（CSI Decoder，CSI 解码器）** 像素窗 1472，刷 B `#506`）：`mnds_y_vsz=0x52705bf` `mnds_y_vph=0x2330d2`（硅清 phase[15:14]；写入 `0x0023b0d2`=(1472<<21)/1320）粘住。仍 **1×8110080**。`camif irq1` 仍 `0x1 0xc 0x2 0x1`。停时 **as0=`0xff000000/0xff528000`**，**as3=`0xfe800000/0xfed28000` 未消费**。`mnds_c_vsz` 仍 `0x293016f`（368→660）。`viol=0` `pix_store=True` `errrec=0`。`g_serial` Device 020 `0525:a4a7` 保住。`streamon_rc=0`。**证伪**：736→1320 改成 1472→1320 解不了第二帧。**保留** 1472→1320（MNDS（MN Down Scaler，M/N 下采样器）是降采样器，禁止再写 736→1320 放大）。禁止再把 **MNDS（MN Down Scaler，M/N 下采样器）** Y V 源当第二帧刀。禁止 `ALIGN_UP FRAME_INCR`。禁止刷 `53dcc70`。
- `#507`（**MNDS（MN Down Scaler，M/N 下采样器）** C V 源配对 Y 像素 1472，刷 B `#507`）：`mnds_c_vsz=0x29302df` `mnds_c_vst=0x2930000` `mnds_c_vph=0x2330d2`（736→660，相位与 Y 同比 1472/1320）粘住。仍 **1×8110080**。`sof_recrop` **n=30** `meas` 一直 **`0x0/0xfff`**。`camif irq1` 仍 `0x1 0xc 0x2 0x1`。`en_reload` **n=4**。停时 **as0=`0xff000000/0xff528000`**，**as3=`0xfe800000/0xfed28000` 未消费**。`mnds_y_vsz=0x52705bf` `viol=0` `pix_store=True` `errrec=0` `wm4=wm5=0x1`。`g_serial` Device 022 `0525:a4a7` 保住。`streamon_rc=0`。**证伪**：C V 368→660 改成 736→660 解不了第二帧。**保留** 736→660（与 Y 1472 配对的色度降采样合同，禁止再写 368→660）。禁止再把 **MNDS（MN Down Scaler，M/N 下采样器）** C V 源当第二帧刀。禁止 `ALIGN_UP FRAME_INCR`。禁止刷 `53dcc70`。

### 0.1 下一刀：第二帧 COMP_DONE（禁止 `0x7e60`）

`#501` 步长 4096 已交出 **一整帧** 线性 NV12（YUV 4:2:0 semi-planar，半平面亮度/色度）。`#502` 恢复前置 IPP（IFE（Image Front End，图像前端）Pixel Path，图像前端像素通路）`pix_store` 后仍一帧。`#503` EOF（End of Frame，帧结束）RESUME 证伪。`#504` SOF（Start of Frame，帧起始）重装 **CSID（CSI Decoder，CSI 解码器）** IPP 裁切窗证伪。`#505` 已 EN 重写 **WM（Write Master，AXI 写通道）** `cfg_0`/`incr`/EN 证伪。`#506` **MNDS（MN Down Scaler，M/N 下采样器）** Y V 1472→1320 证伪（保留降采样合同）。`#507` **MNDS（MN Down Scaler，M/N 下采样器）** C V 736→660 证伪（保留色度合同）。`vfe_buf_done` 已写下一帧 **IMAGE_ADDR** + **RUP（Register Update，寄存器更新）** `0x41`，as3 有 IOVA，as0 不跟。后置同一套 ADDR-only + bit7 能连出三帧。门仍是 **≥3×8110080**（有用宽仍 2320），UV（chroma，色度）~128，`viol≠19`。下一刀查 **CAMIF（Camera Interface，相机接口）** 第二次 SOF（Start of Frame，帧起始）之后为何不数行到 epoch：哪一级 **CLC（Camera Logic Core，相机逻辑核）** 在交完第一帧后停写，不是再改 **MNDS（MN Down Scaler，M/N 下采样器）** V 源。禁止再关 `pix_store`。禁止再脉冲 RESUME。禁止再 SOF（Start of Frame，帧起始）只重写 HCROP/VCROP（Horizontal/Vertical Crop，水平/垂直裁切）。禁止再把 Code Aurora Forum `update_wm` 当第二帧刀。禁止再写 736→1320 放大。禁止再写 368→660。禁止 `ALIGN_UP FRAME_INCR`。禁止 Dual-IFE（dual Image Front End，双图像前端）`COMP_CFG`。禁止 chroma 7。禁止 `0x2e58` EN。禁止 Crop 640。禁止刷 `53dcc70`。

```mermaid
flowchart LR
  NOW["现在 · #507 MNDS C V 736 证伪<br/>第二 SOF（Start of Frame，帧起始）无 epoch"]
  CUT["下一刀 · CAMIF（Camera Interface，相机接口）第二 SOF 后为何不数行"]
