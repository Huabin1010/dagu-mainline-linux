...
- **GNOME 点连接失败**：Settings 走 `Device1.Connect`。已绑定低功耗蓝牙（Bluetooth Low Energy，低功耗蓝牙）人机接口设备（Human Interface Device，人机接口设备）先进接受列表被动扫描，不发 `LE Create Connection`。点失败后内核留下 `handle 3840+ state 5`；再点就 `Pair device failed: Timeout (0x08)`。`Add Device` 随机地址回 `Failed (0x03)` 会重下 `htbtfw20.tlv`，`Pairable` 掉成 no。正路：`AlwaysPairable=true`；`dagu-bt-hid-host.sh` 在 udev `change` / 主机控制器接口（Host Controller Interface，主机控制器接口）UP 后再套 bondable，并对已绑定设备写 `Trusted=yes`（GNOME（GNU Network Object Model Environment，GNOME 桌面）「未设置」是 `!Paired && !Trusted`；`bluetoothctl pair` 不写 Trusted，行一直未设置）。取消接受列表 / 未分配句柄的低功耗蓝牙（Bluetooth Low Energy，低功耗蓝牙）连接不要发 `LE_CREATE_CONN_CANCEL`。K380 的 `ReconnectMode=device`：GNOME（GNU Network Object Model Environment，GNOME 桌面）再点连接不再发主机控制器接口（Host Controller Interface，主机控制器接口）`Create Connection`（实机计数 0），保持 page scan，等按键寻呼进来；`bluetoothd` 由 `dagu-build-bluez-hid.sh` 重编。未设置转圈：Settings 先关 setup-mode 再 `Device1.Pair`。已绑定设备的 `Pair` 回 `org.bluez.Error.AlreadyExists`，库存 `setup_device` 当失败，从不 Trusted、从不 Connect，行转到超时。正路：`dagu-build-gnome-bt.sh` 重装 `libgnome-bluetooth` / `libgnome-bluetooth-ui`，AlreadyExists 当 Pair 成功，停扫描后排空 Discovering 再 Pair/Connect，Connect 重试 25 秒。内核 `hci_connect_le_scan` 还要再等一条广播时鼠标已经不在广播里，主机控制器接口（Host Controller Interface，主机控制器接口）从不发低功耗蓝牙（Bluetooth Low Energy，低功耗蓝牙）`LE Create Connection`。正路：逻辑链路控制与适配协议（Logical Link Control and Adaptation Protocol，逻辑链路控制与适配协议）用户点击走 `hci_connect_le`，控制器立刻对这个地址建链。`#510` B 槽：`Device1.Connect` `23:15:1D:17:21:32` 立刻 `LE Extended Create Connection`，`LE Enhanced Connection Complete` handle 2/3，`Connected=yes`，`/dev/input/event14`=`BT5.0 Mouse`，`hidraw4` 通用人机接口设备（Human Interface Device，人机接口设备）。未配对 `77:26:1F:A4:73:C7`（黄华彬的 REDMI K80 Ultra）同样发出建链并走进安全管理协议（Security Manager Protocol，安全管理协议）配对请求。设置页还在 Inquiry 时，低功耗蓝牙（Bluetooth Low Energy，低功耗蓝牙）建链会被时分双工（Time Division Duplex，时分双工）电台取消（`le-connection-abort-by-local`）；正路：`hci_le_create_conn_sync` 先 `INQUIRY_CANCEL`（与 ACL（Asynchronous Connection-Less，异步无连接）建链相同），`hci_start_discovery_sync` 在建链未完成时返回 `-EBUSY`。`#520` 已含这条。GNOME（GNU Network Object Model Environment，GNOME 桌面）面板继续开着。
- **刷核**：clock offset / Central / 交错 page scan / HID sniff / ACL 清理 / 6390 去掉 `HCI_QUIRK_SIMULTANEOUS_DISCOVERY` / 低功耗蓝牙（Bluetooth Low Energy，低功耗蓝牙）取消清僵尸 / 逻辑链路控制与适配协议（Logical Link Control and Adaptation Protocol，逻辑链路控制与适配协议）`hci_connect_le` / `INQUIRY_CANCEL` 都在 `apply-overlays.sh` → 运行核 `#520` B 槽。用户态 `main.conf` 跟 rootfs 脚本走；设置页补丁跟 `dagu-build-gnome-bt.sh`。
