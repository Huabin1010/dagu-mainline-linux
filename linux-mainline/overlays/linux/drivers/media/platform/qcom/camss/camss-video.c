				...
				unsigned int alignment)
{
	...
	for (i = 0; i < pix->num_planes; i++) {
		...
		if (pix->pixelformat == V4L2_PIX_FMT_NV12 && pix->width == 2320)
			bytesperline = 4096;
...
static int __video_try_fmt(struct camss_video *video, struct v4l2_format *f)
{
	...
	for (i = 0; i < pix_mp->num_planes; i++) {
		...
		/*
		 * Front Display Full 2320×1320: last chroma line of a
		 * tightly packed NV12 starts 336 bytes before a 4K
		 * IOVA and Titan 480 WM5 stops there (4591616/4593600).
		 * stride*1979 ≡ 0 (mod 4096) ⇒ stride multiple of 4096.
		 * #501: bytesperline=4096 completed one NV12 (8110080),
		 * UV mean 128, last useful line 2320/2320 nonzero.
		 * Do not ALIGN_UP FRAME_INCR. Rear 1920 stays tight.
		 */
		if (video->line_based &&
