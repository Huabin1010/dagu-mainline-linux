...
static int fopen_mode_to_oflags(const char *mode)
{
	int oflags = O_RDONLY;
				...
				struct fastrpc_io_buffer *outbufs)
{
	...
	if (ret) {
		fprintf(stderr, "Could not close %u: %s\n", *first_in,
			strerror(-ret));
		return AEE_EFAILED;
	}
			       ...
			       struct fastrpc_io_buffer *outbufs)
{
	...
	n = inbufs[1].s;
	if (n > first_in->buf_size)
		n = first_in->buf_size;

	ret = hexagonfs_write(ctx->fds, first_in->fd, n, inbufs[1].p);
	if (ret < 0) {
			      ...
			      struct fastrpc_io_buffer *outbufs)
{
	...
	name = inbufs[1].p;
	mode = inbufs[2].p;
	oflags = fopen_mode_to_oflags(mode);

	fd = hexagonfs_openat(ctx->fds, ctx->rootfd, ctx->rootfd, name, oflags);
	if (fd < 0) {
		fprintf(stderr, "Could not fopen %s (%s): %s\n",
					...
					struct fastrpc_io_buffer *outbufs)
{
	...
	fd = hexagonfs_openat(ctx->fds, ctx->rootfd, dirfd, inbufs[3].p,
			      fopen_mode_to_oflags(inbufs[4].p));
	if (fd < 0) {
	...
	if (rw_mode == 'w' || rw_mode == 'a')
		fprintf(stderr, "fopen_with_env write %s mode=%s fd=%d\n",
			(const char *) inbufs[3].p,
			(const char *) inbufs[4].p, fd);
				...
				struct fastrpc_io_buffer *outbufs)
{
	...
	ret = hexagonfs_rename(ctx->fds, ctx->rootfd, inbufs[1].p, inbufs[2].p);
	if (ret) {
				 ...
				 struct fastrpc_io_buffer *outbufs)
{
	...
	ret = hexagonfs_openat(ctx->fds, ctx->rootfd, ctx->rootfd, inbufs[1].p, 0);
	if (ret < 0) {
		fprintf(stderr, "Could not open %s: %s\n",
				(const char *) inbufs[1].p,
				strerror(-ret));
			      ...
			      struct fastrpc_io_buffer *outbufs)
{
	...
	fd = hexagonfs_openat(ctx->fds, ctx->rootfd, ctx->adsp_library_dirfd, pathname, 0);
	if (fd < 0) {
		fprintf(stderr, "Could not open %s: %s\n",
