...
/*
 * SEE fwrite of sns_diag_filter.json is ~5 KiB; sm8250_sx932x JSON ~8 KiB.
 * Android sscrpcd sizes listener next2 OUT to tens of KiB and grows.
 */
#define DAGU_LISTENER_INBUFS_MIN 65536u
				  ...
				  struct fastrpc_io_buffer **decoded)
{
	...
	char *inbufs = NULL;
	char *outbufs = NULL;
	uint32_t inbufs_cap = DAGU_LISTENER_INBUFS_MIN;
	uint32_t inbufs_len = 0;
	...
	inbufs = malloc(inbufs_cap);
	if (inbufs == NULL) {
		perror("Could not allocate listener inbufs");
		free(outbufs);
		return -1;
	}
	...
	for (grow = 0; grow < 4; grow++) {
		...
		/*
		 * next2 copied min(cap, needed) and set inbufs_len to the
		 * full invoke. Grow and re-fetch; AEE_EBUFFERTOOSMALL means
		 * the truncated payload was not a completed reverse invoke.
		 */
		fprintf(stderr, "listener next2 grow inbufs %u -> %u\n",
			inbufs_cap, inbufs_len);
		if (inbufs_len > 8u * 1024u * 1024u) {
			fprintf(stderr, "listener next2 inbufs %u rejected\n",
				inbufs_len);
			ret = -1;
			goto err_free;
		}
		...
		if (inbufs == NULL) {
			perror("Could not grow listener inbufs");
			ret = -1;
			goto err_free;
		}
		inbufs_cap = inbufs_len;
	...
	if (inbufs_len > inbufs_cap) {
		fprintf(stderr, "listener next2 inbufs %u still larger than %u\n",
