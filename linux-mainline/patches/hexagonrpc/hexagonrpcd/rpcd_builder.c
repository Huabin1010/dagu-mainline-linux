...
{
	char *acdbdata, *dsp_libs, *sns_cfg, *sns_persist, *sns_reg_config, *socinfo;
	size_t n_prefix;
	...
	sns_persist = malloc(n_prefix + strlen(SENSORS_PERSIST_REG) + 1);
	...
	if (sns_persist != NULL) {
		...
		strcat(sns_persist, SENSORS_PERSIST_REG);
	}
