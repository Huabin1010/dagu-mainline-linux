#!/usr/bin/env python3
"""Type B holes: cheap gpu_busy_percent only. Never open dri/0/gpu hangrd rd."""
...
def on_device(seconds=10.0):
    ...
    def sampler():
        while not stop.is_set():
            try:
                ...
                busy = None
            snaps.append((time.clock_gettime(time.CLOCK_MONOTONIC), busy))
            time.sleep(0.002)
    ...
    for a, b in zip(kick, kick[1:]):
        ...
        win = [s for s in snaps if a <= s[0] <= b]
        busies = [s[1] for s in win if s[1] is not None]
        z = sum(1 for x in busies if x == 0)
        holes.append({
            "gap_ms": round(gap, 1),
            "kind": kind,
            ...
            "busy_zero": z,
            "busy_zero_frac": round(z / len(busies), 2) if busies else None,
        })
    ...
    Path("/tmp/dagu-gpu-busy-hole.json").write_text(json.dumps(out, indent=2) + "\n")
...
def host_main():
    ...
    here = Path(__file__)
    subprocess.run(scp + [str(here), f"root@{HOST}:/tmp/dagu-gpu-busy-hole-probe.py"], check=True)
    r = subprocess.run(ssh + ["python3 /tmp/dagu-gpu-busy-hole-probe.py"] + extra, check=False)
    dest_dir = ROOT / "out" / "display-stress"
    dest_dir.mkdir(parents=True, exist_ok=True)
    stamp = time.strftime("%Y%m%d-%H%M%S")
    dest = dest_dir / f"dagu-gpu-busy-hole-{stamp}.json"
    ...
    if r.returncode != 0:
        ...
        return r.returncode
    subprocess.run(scp + [f"root@{HOST}:/tmp/dagu-gpu-busy-hole.json", str(dest)], check=False)
    if dest.is_file():
