#!/usr/bin/env python3
"""nview-late: qcb vs nview, main syscall at +20ms, GSource count; ifn-idle first sched.

Never hook 0x1c4404 / cave. ifn via 0x1c4388 cbz (stock, x1=next).
qcb is flipped_in_impl bl queue_callback 0x1bd35c (KMS thread).
gs is glib dispatch blr 0x606f4 — count only, no duration pair.

From host: python3 linux-mainline/scripts/dagu-nview-starve-probe.py --host
"""
...
def read_syscall(tid):
    try:
        raw = Path(f"/proc/{tid}/syscall").read_text().strip()
        wchan = Path(f"/proc/{tid}/wchan").read_text().strip()
    except OSError:
        return {"raw": "gone", "wchan": "gone"}
    if raw == "running":
        return {"nr": -1, "name": "running", "wchan": wchan, "raw": raw}
    parts = raw.split()
    try:
        nr = int(parts[0], 0)
    except (ValueError, IndexError):
        return {"raw": raw, "wchan": wchan}
    return {
        "nr": nr,
        "name": SYSC.get(nr, f"nr{nr}"),
        "wchan": wchan,
        "raw": raw[:80],
    }
...
def install(shell):
    ...
    try:
        ...
        os.write(fd, f"p:dagu_qcb {mutter}:0x1bd35c\n".encode())
        os.write(fd, f"p:dagu_wlsched {mutter}:0x16517c\n".encode())
        os.write(fd, f"p:dagu_au {mutter}:0x1696c0\n".encode())
        os.write(fd, f"p:dagu_sched {clutter}:0x675a0 lr=%x30\n".encode())
...
def on_device(seconds=12.0):
        ...
        while time.clock_gettime(time.CLOCK_MONOTONIC) < t_end:
            ...
            while "\n" in buf:
                ...
                if "dpu_enc_kickoff:" in line:
                    ...
                    nview.append(ts)
                    if cur_kick is not None and not saw_nview:
                        saw_nview = True
                    ...
                    qcb.append(ts)
                elif "dagu_wlsched:" in line:
                    wlsched.append(ts)
                elif "dagu_au:" in line:
                    au.append(ts)
                elif "dagu_sched:" in line:
                    sched.append((ts, parse_hex_field(line, "lr") or 0))
                ...
                if (
                    ...
                    sys_wins.append((cur_kick, ts, read_syscall(shell)))
                    sampled = True
    finally:
        pipe.close()
        clear()
    ...
    gs_by_kick = {t: (n, top) for t, n, top in gs_wins}
    sys_by_kick = {t: (st, info) for t, st, info in sys_wins}
    ...
    for a, b in zip(kick, kick[1:]):
        ...
        if n_dt is not None and ifn_dt is not None:
            ...
            kind = "nview-late"
        gs_n, top = gs_by_kick.get(a, (None, []))
        names = []
        for src, c in top:
            names.append({"n": c, "name": src_name(shell, src)})
        sy = sys_by_kick.get(a)
        first_sched = None
        first_lr = None
        for t, lr in sched:
            dt = (t - a) * 1000.0
            if 0 <= dt <= 280:
                first_sched = round(dt, 2)
                first_lr = hex(lr)
                break
        holes.append({
            "gap_ms": round(gap, 1),
            "kind": kind,
            "qcb": first_after(qcb, a, -2, 280),
            "nview": first_after(nview, a, 0, 280),
            "ifn": first_after(ifn, a, 0, 280),
            "wlsched": first_after(wlsched, a, 0, 280),
            "au": first_after(au, a, 0, 280),
            "sched0": first_sched,
            "sched_lr": first_lr,
            "gs_before_nview": gs_n,
            "gs_top": names,
            "sys20": None if sy is None else {
                "at_ms": round((sy[0] - a) * 1000.0, 2),
                **sy[1],
            },
        })
    ...
    Path("/tmp/dagu-nview-starve.json").write_text(json.dumps(out, indent=2) + "\n")
...
def host_main():
    ...
    here = Path(__file__)
    subprocess.run(scp + [str(here), f"root@{HOST}:/tmp/dagu-nview-starve-probe.py"], check=True)
    r = subprocess.run(ssh + ["python3 /tmp/dagu-nview-starve-probe.py"] + extra, check=False)
    dest_dir = ROOT / "out" / "display-stress"
    dest_dir.mkdir(parents=True, exist_ok=True)
    stamp = time.strftime("%Y%m%d-%H%M%S")
    dest = dest_dir / f"dagu-nview-starve-{stamp}.json"
    if r.returncode != 0:
        ...
        return r.returncode
    subprocess.run(scp + [f"root@{HOST}:/tmp/dagu-nview-starve.json", str(dest)], check=False)
    if dest.is_file():
