  ...
  )
{
  ...
  //
  // dagu: ignore UFS/partition auto-entries ("Misc Device", "Hard Drive", …).
  // Only firmware-volume apps belong in this bring-up menu.
  //
  {
    ...
    IsFv = FALSE;
    for (Node = BootOption->FilePath; (Node != NULL) && !IsDevicePathEnd (Node);
  ...
  )
{
  ...
  //
  // dagu: fingerprint MUST live in the popup title — ConOut Print is wiped by
  // DrawBootPopupMenu. If you do not see UEFI-ONLY here, wrong image is running.
  //
  BootMenuData->TitleToken[0] = HiiSetString (
                                  gStringPackHandle,
                                  0,
                                  L"*** UEFI-ONLY *** Vol-/+  Power=OK",
                                  NULL
                                  );
  ...
  BootMenuData->HelpToken[0] = HiiSetString (
                                 gStringPackHandle, 0,
                                 L"Vol- / Vol+ = move selection", NULL);
  BootMenuData->HelpToken[1] = HiiSetString (
                                 gStringPackHandle, 0,
                                 L"Power = confirm", NULL);
  BootMenuData->HelpToken[2] = HiiSetString (
                                 gStringPackHandle, 0,
                                 L"ESC = exit menu", NULL);
  ...
  )
{
  ...
  for (ItemNum = 0, Index = 0; Index < BootOptionCount; Index++) {
    ...
    if (ItemNum++ == SelectItem) {
      ...
      if (EFI_ERROR (BootOptions[Index].Status) && (gST->ConOut != NULL)) {
        Print (
...
/**
  dagu: poll SimpleTextIn + SimpleTextInEx on every handle.
  Qualcomm ButtonsDxe often exposes only TextInEx and is not gST->ConIn.
**/
STATIC
EFI_STATUS
DaguReadAnyKey (
  ...
  )
{
  ...
  if (gST->ConIn != NULL) {
    ...
    if (!EFI_ERROR (Status) &&
        ((Key->ScanCode != SCAN_NULL) || (Key->UnicodeChar != CHAR_NULL)))
    {
      return EFI_SUCCESS;
    }
  }
  ...
  //
  // TextInEx first — ButtonsDxe on MSM typically only implements Ex.
  //
  Status = gBS->LocateHandleBuffer (
                  ByProtocol,
  ...
  if (!EFI_ERROR (Status)) {
    for (Index = 0; Index < HandleCount; Index++) {
      ...
      //
      // Qualcomm ButtonsDxe samples GPIOs from the WaitForKeyEx notify path.
      // Pump CheckEvent before ReadKeyStrokeEx or keys stay EFI_NOT_READY.
      //
      if (TextInEx->WaitForKeyEx != NULL) {
...
{
  if ((gST->ConOut == NULL) || (Key == NULL)) {
    return;
  }

  //
  // Bottom-line feedback so bring-up can see whether ButtonsDxe fires.
  //
  gST->ConOut->SetCursorPosition (gST->ConOut, 0, gST->ConOut->Mode->CursorRow > 0 ? gST->ConOut->Mode->CursorRow : 0);
  Print (
    L"\r  key sc=0x%04x uni=0x%04x   Vol-/+ move  Power=OK     ",
    Key->ScanCode,
    (UINT16)Key->UnicodeChar
    );
}
  ...
  )
{
  ...
  //
  // dagu: do NOT RefreshAllBootOption here — that re-creates dozens of
  // "Misc Device" entries from every UFS partition and undoes PlatformBm prune.
  // ConnectAll is also skipped; Shell / Simple Init bring their own devices.
  //
  {
    UINTN       ExCount;
    EFI_HANDLE  *ExHandles;

    ExCount = 0;
    ExHandles = NULL;
    if (!EFI_ERROR (gBS->LocateHandleBuffer (
            ByProtocol, &gEfiSimpleTextInputExProtocolGuid, NULL, &ExCount,
            &ExHandles)))
    {
      FreePool (ExHandles);
    }
    ...
    if (gST->ConOut != NULL) {
      Print (L"  TextInEx=%u (see popup title)\r\n", ExCount);
    }
  }
  ...
  while (!ExitApplication) {
    //
    // Poll all TextIn/TextInEx; do not block on ConIn alone.
    //
    gBS->Stall (15000);
    ...
    DaguShowKeyDebug (&Key);

    if (DaguKeyIsConfirm (&Key)) {
      gST->ConOut->ClearScreen (gST->ConOut);
      ...
      //
      // Unknown hardware scan code: still move selection so bring-up is usable
      // while we learn the ButtonsDxe mapping from on-screen debug.
      //
      SelectItem = BootMenuData.SelectItem == BootMenuData.ItemCount - 1 ? 0 : BootMenuData.SelectItem + 1;
      BootMenuSelectItem (SelectItem, &BootMenuData);
    }
