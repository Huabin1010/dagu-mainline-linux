...
#define DAGU_BOOT_HINT  L"Vol+/- move  Power confirm"

#define DAGU_BOOT_OPT_INIT  L"Simple Init"
#define DAGU_BOOT_OPT_SHELL L"UEFI Shell"

STATIC
VOID
PlatformDaguClearBootNext (
  VOID
  )
{
  gRT->SetVariable (
...
STATIC
BOOLEAN
PlatformDaguIsFirmwareVolumeBootOption (
  ...
  )
{
  ...
  for (Node = Option->FilePath; !IsDevicePathEnd (Node);
...
/**
  Drop auto-enumerated UFS/partition "Misc Device" entries from NVRAM BootOrder.
  Keep only firmware-volume apps (Simple Init / Shell / menu).
**/
STATIC
VOID
PlatformDaguPruneNonFirmwareBootOptions (
  VOID
  )
{
  EFI_BOOT_MANAGER_LOAD_OPTION  *BootOptions;
  ...
  for (Index = 0; Index < BootOptionCount; Index++) {
    if ((BootOptions[Index].Attributes & LOAD_OPTION_CATEGORY) ==
        LOAD_OPTION_CATEGORY_APP)
    {
      continue;
    }

    if (PlatformDaguIsFirmwareVolumeBootOption (&BootOptions[Index])) {
      continue;
    }

    DEBUG ((
        DEBUG_ERROR,
        "Dagu: prune boot option \"%s\"\n",
...
STATIC
VOID
PlatformDaguMoveBootOptionToFront (
  ...
  )
{
  ...
  BootOptions = EfiBootManagerGetLoadOptions (&BootOptionCount, LoadOptionTypeBoot);
  for (Index = 0; Index < BootOptionCount; Index++) {
    if (StrCmp (BootOptions[Index].Description, Description) == 0) {
      EfiBootManagerAddLoadOptionVariable (&BootOptions[Index], 0);
      DEBUG ((
          DEBUG_ERROR,
          "Dagu: BootOrder front -> \"%s\" (Boot%04x)\n",
          Description,
          BootOptions[Index].OptionNumber
          ));
      break;
...
/**
  Always show a selectable boot list (Simple Init / Shell).
  Never fall through to auto-boot Shell.

  Uses BootManagerMenuApp (popup), not UiApp FrontPage.
**/
STATIC
VOID
PlatformDaguEnterInteractiveBootMenu (
  VOID
  )
{
  EFI_BOOT_MANAGER_LOAD_OPTION  Menu;
  EFI_STATUS                    Status;

  PlatformDaguClearBootNext ();

  //
  // Menu order (first = highlighted): Simple Init, Shell.
  //
  PlatformDaguMoveBootOptionToFront (DAGU_BOOT_OPT_SHELL);
  PlatformDaguMoveBootOptionToFront (DAGU_BOOT_OPT_INIT);

  Status = EfiBootManagerGetBootManagerMenu (&Menu);
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_ERROR, "Dagu: BootManagerMenu missing %r — fallback loop\n", Status));
    for (;;) {
      if (gST->ConOut != NULL) {
        Print (L"\r\nBoot menu unavailable (%r)\r\n", Status);
        Print (L"Trying Simple Init / Shell...\r\n");
      }

      PlatformDaguBootByDescription (DAGU_BOOT_OPT_INIT);
      PlatformDaguBootByDescription (DAGU_BOOT_OPT_SHELL);
      gBS->Stall (2000000);
    }
  }
  ...
  for (;;) {
    if (gST->ConOut != NULL) {
      ...
      //
      // Unique on-screen fingerprint — if you do not see this string, the
      // tablet is NOT running artifacts/boot-dagu-latest.img.
      //
      Print (L"\r\n  *** DAGU BUILD UEFI-ONLY ***\r\n");
      Print (L"  Xiaomi Pad 5 Pro 12.4 (dagu)\r\n");
      Print (L"  %s\r\n", DAGU_BOOT_HINT);
      Print (L"  1) Simple Init\r\n");
      Print (L"  2) UEFI Shell\r\n\r\n");
    }

    EfiBootManagerBoot (&Menu);
    // Returned after ESC or a selected app exited — show menu again.
  }
}
...
VOID PlatformRegisterOptionsAndKeys(VOID)
{
  ...
  //
  // Map volume keys and power to Boot Manager / Simple Init hotkeys.
  //
  VolUp.ScanCode     = SCAN_VOLUME_UP;
  VolUp.UnicodeChar  = CHAR_NULL;
  VolDown.ScanCode   = SCAN_VOLUME_DOWN;
  VolDown.UnicodeChar = CHAR_NULL;
  Power.ScanCode     = SCAN_SUSPEND;
  Power.UnicodeChar  = CHAR_NULL;
  ...
  Status = EfiBootManagerAddKeyOptionVariable (
      NULL, (UINT16)OptionSimpleInit, 0, &VolUp, NULL);
#endif
  Status = EfiBootManagerAddKeyOptionVariable (
      NULL, (UINT16)BootOption.OptionNumber, 0, &VolDown, NULL);
  Status = EfiBootManagerAddKeyOptionVariable (
      NULL, (UINT16)BootOption.OptionNumber, 0, &Power, NULL);
...
VOID EFIAPI PlatformBootManagerBeforeConsole(VOID)
{
  ...
  //
  // dagu: register Qualcomm ButtonsDxe keypad vendor device path as ConIn.
  // GUID matches Silicon/Qualcomm/QcomPkg/Include/Configuration/BootDevices.h
  //
  {
    STATIC struct {
    ...
    EfiBootManagerUpdateConsoleVariable (
        ConIn, (EFI_DEVICE_PATH_PROTOCOL *)&KeyPadPath, NULL);
  }

  //
  // dagu: register ALL TextInEx devices as ConIn (not just handles[1]).
  // ButtonsDxe is often not the second handle — missing it breaks volume keys.
  //
  {
    EFI_HANDLE *              Handles;
    UINTN                     HandleCount;
    UINTN                     Index;
    EFI_DEVICE_PATH_PROTOCOL *DevicePath;
    EFI_STATUS                LocStatus;

    LocStatus = gBS->LocateHandleBuffer (
        ByProtocol, &gEfiSimpleTextInputExProtocolGuid, NULL, &HandleCount,
        &Handles);
    if (!EFI_ERROR (LocStatus)) {
...
VOID EFIAPI PlatformBootManagerAfterConsole(VOID)
{
  ...
  //
  // Do NOT refresh/enumerate block boot targets — on UFS tablets that
  // creates dozens of useless "Misc Device" entries from every partition.
  //
  PlatformDaguPruneNonFirmwareBootOptions ();

  //
  // Boot options for the interactive menu only.
  //
#ifdef ENABLE_SIMPLE_INIT
  PlatformRegisterFvBootOption (
      &gSimpleInitFileGuid, DAGU_BOOT_OPT_INIT, LOAD_OPTION_ACTIVE);
#endif
  PlatformRegisterFvBootOption (
      &gUefiShellFileGuid, DAGU_BOOT_OPT_SHELL, LOAD_OPTION_ACTIVE);
...
VOID EFIAPI PlatformBootManagerUnableToBoot(VOID)
{
  if (gST->ConOut != NULL) {
    gST->ConOut->ClearScreen (gST->ConOut);
    Print (L"\r\n[Bds] boot option failed — back to menu\r\n");
  }
